...
APIやDBの一時障害が起きた場合、既に表示しているルーティーンやログは保持したままエラーを表示します。画面上の「再試行」から再読み込みでき、保存操作はサーバーの成功応答を受けた場合だけ画面へ反映します。セッション切れ（401）の場合は古いデータを消去してログイン画面へ戻します。ログイン・登録のレート制限（429）や予期しない画面エラー（500）には、次に取る操作を表示します。
