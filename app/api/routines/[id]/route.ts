...
import { deactivateRoutineForUser, parseRoutineInput, reactivateRoutineForUser, toggleRoutineLog, updateRoutineForUser } from "@/lib/routine-service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (body.action === "toggle" && typeof body.date === "string") return NextResponse.json({ log: await toggleRoutineLog(user.id, id, body.date) });
