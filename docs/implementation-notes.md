...
クライアントの保存操作は成功レスポンスを受けてから状態を更新するため、API / DB障害時に成功表示へ進みません。通信失敗時は既存データを保持してエラーと再試行導線を表示します。401を受けた場合はユーザー・ルーティーン・ログを即時消去してログイン画面へ戻し、429は待機を促すメッセージ、500は内部情報を隠した一般メッセージを返します。Next.jsの`error.tsx` / `global-error.tsx`にも再試行とトップへの復旧導線を用意しています。
