...
本番運用はDocker self-host + 外部HTTPS reverse proxyを前提にする。`compose.prod.yaml`の`app`、`postgres`、one-shotの`migrate`を分離し、`docker compose ... build` → `docker compose ... run --rm migrate` → `docker compose ... up -d --no-deps app`の順に実行する。image buildではDB migrationを実行しない。Production / Previewの`DATABASE_URL`と`DEPLOY_ENV`は別の設定にし、`RELEASE_COMMIT_SHA`、`RELEASE_BRANCH`、`/api/health`で稼働commitを追跡する。Rate LimitはVercel専用ヘッダーに依存せず、管理下のreverse proxyで正規化された`x-forwarded-for` / `x-real-ip`だけを、`TRUST_PROXY_HEADERS=true`の場合に参照する。
